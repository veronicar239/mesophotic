...
Summary of publication formats
```{r echo=FALSE}
publs %>% 
...
Overall summary of different types of publications
```{r}
publs %>% 
...
Summary of validated publications
```{r message=FALSE}
publs %>% 
  filter(publication_type == "scientific" & publication_format == "article" | publication_format == "chapter") %>% 
    group_by(publication_type, publication_format, validated) %>% 
      dplyr::summarise(count=n())
```
*validated = FALSE scientific articles are typically early (older) papers that are missing both abstract and full text, and thus metadata cannot be entered (nor validated)*

Years with top publication counts
*Interesting peak during international coral reef conference (ICRS) in 2016 and 2021*
```{r}
publs %>% 
...
Look at what other fields are studied in publications with Long-term monitoring tag
```{r}
publs_LTM_fields %>%
    group_by(fields) %>% 
      dplyr::summarise(count=n()) %>% 
...
```{r}
# use validated == "true" for file obtained directly from website,
# or validated == "TRUE" for static downloaded CSV file
# same for included_in_stats variable
...
MCE
```{r}
publs.mce <- subset(publs, mce == "TRUE") #mce == "true"
...
TME
```{r}
publs.tme <- subset(publs, tme == "TRUE") #tme == "true"
...
144 publications (scientific articles) with new species
*Scleractinia (Hard Corals) 3 articles, for a total of 4 new species*
```{r}
publs_focusgroups %>% 
  filter(new_species == "TRUE") %>% #new_species == "true"
    group_by(focusgroups, publication_format) %>%  
      dplyr::summarise(count=n()) %>% 
        arrange(desc(count))
```

Overall count of focus groups
```{r}
publs_focusgroups %>%
    group_by(focusgroups) %>% 
      dplyr::summarise(count=n()) %>% 
...
publs_fields_CoralFishOctocoral$focusgroups <- recode(publs_fields_CoralFishOctocoral$focusgroups, 'Scleractinia (Hard Corals)' = "Corals")

publs_fields_CoralFishOctocoral$focusgroups <- factor(publs_fields_CoralFishOctocoral$focusgroups, levels = c("Octocorals", "Fishes", "Corals"))
...
barplot_fields <- publs_fields_CoralFishOctocoral %>% 
  ...
  group_by(fields, focusgroups) %>% 
      dplyr::summarise(count=n()) %>%
        ggplot(aes(x = count, y = fields, fill = str_wrap(focusgroups))) +
          geom_bar(stat="identity", position="dodge") + #, fill = "grey"
          #theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
          #scale_fill_viridis(discrete=TRUE, name="") +
          scale_fill_manual("Focus group", values=c("#fde725", "#440154", "#25858EFF")) +
          scale_x_continuous(limits = c(0, 60)) +
          xlab("Publications") +
          #ylab("") +
          #labs(x = "Percent", y = "Fields", title = "TME") +
           theme_minimal() +  
           theme(panel.background = element_rect(colour = "grey50"), axis.text.y=element_blank(), axis.title.y=element_blank(), legend.position = c(.75,.1))
...
ggsave(plot, file="Fig Fields emerging rideplot barplot combo_focusgroups_Final.pdf", width = 8, height = 6, dpi = 150, units = "in", device="pdf")
#ggsave(plot, file="Fig Fields emerging rideplot barplot combo_focusgroups_Final.eps", width = 8, height = 6, dpi = 150, units = "in", device="eps")
#ggsave(plot, file="Fig Fields emerging rideplot barplot combo_focusgroups_Final.jpg", width = 8, height = 6, dpi = 150, units = "in", device="jpg")
#ggsave(plot, file="Fig Fields emerging rideplot barplot combo_focusgroups_Final.tiff", width = 8, height = 6, dpi = 150, units = "in", device="tiff")
...
```{r}
publs.mce <- subset(publs, mce == "TRUE" & tme == "FALSE") #mce == "true" & tme == "false"
publs.mce <- droplevels(publs.mce)
```
...
```{r}
#Separate by periods, unp until 2008 and 2009-2023
publs.mce_locations$range <- ifelse(publs.mce_locations$publication_year <= 2008, '<2008',
...
publs_mce_2023 <- droplevels(publs_mce_2023)
...
*However, there is overlap of locations that are present in both Before 2008 and 2009-2023 data*

***Cross check to only obtain unique locations***

In summary, 54 MCE locations were both in Before 2008 and 2008-2023 categories
Therefore 97 new locations 2008-2023 - 53 shared locations = 44 new MCE locations between 2008-2023
Note: 13 unique MCE locations that were only from Before 2008 (but not studied again in 2008-2023)

Grand total = 13 unique locations Before 2008 + 53 shared locations two time periods + 44 new MCE locations 2008-2023 + 1 La Perouse* + 1 Singapore* = TOTAL 112 unique MCE locations

*La Perouse (publication 1642 - had both MCE and TME tags, but manually reviewed and it is a valid new location in 2023)
*Singapore (often without "mesophotic" tag because research at shallow depths, but included in database because low-light reef environment is relevant to mesophotic science)
```{r}
...
```{r}
publs.tme <- subset(publs, tme == "TRUE" & mce == "FALSE") #tme == "true" & mce == "false"
publs.tme <- droplevels(publs.tme)
```
...
```{r}
#Separate by periods, up until 2008 and 2009-2023
publs.tme_locations$range <- ifelse(publs.tme_locations$publication_year <= 2008, '<2008',
...
publs_tme_2008 <- droplevels(publs_tme_2008)
n_distinct(publs_tme_2008$locations)
```
*1 unique location from before 2008 ("Mexico - Baja California")*

```{r}
#2009-2023
publs_tme_2023 <- subset(publs.tme_locations, range == "<2023")
publs_tme_2023 <- droplevels(publs_tme_2023)
n_distinct(publs_tme_2023$locations)
```

In summary, 18 TME locations were both in Before 2008 and 2008-2023 categories
Therefore 41 new locations 2008-2023 - 18 shared locations = 23 new TME locations between 2008-2023

Therefore, 1 unique Before 2008 locations + 18 shared locations two time periods
+ 23 new 2008-2023 locations = 42 locations total
```{r}
#find duplicates between two data frames
...
publs_locations$range <- ifelse(publs_locations$publication_year <= 2008, '<2008',
...
```{r}
publs_locations <- subset(publs_locations, original_data == "TRUE") #original_data == "true"
